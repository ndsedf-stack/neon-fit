...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#010101] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30 pb-20">
      ...
      {/* HEADER */}
      <header className="header-cockpit">
        <div className="max-w-md mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <button 
            onClick={resetDemo}
            className="touch-pulse w-8 h-8 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 transition-all active:scale-95"
          >
            <RotateCcw size={14} />
          </button>
        </div>
      ...
      {/* MAIN CONTENT */}
      <main className="container-cockpit relative z-10">
        <div className="stats-card animate-section" style={{ animationDelay: '0.1s' }}>
          <NeonTracker score={score} sessions={{ current: sessions, max: maxSessions }} sets={{ current: sets, max: maxSets }} status={score > 80 ? 'consistent' : 'warning'} />
        </div>

        <div className="stats-card animate-section" style={{ animationDelay: '0.2s' }}>
          <WeeklyProgress />
        </div>

        <div className="stats-card animate-section animate-float" style={{ animationDelay: '0.3s' }}>
          <VolumeGauge volume={volume} maxVolume={25000} stats={{ sets, tut: 2450, sessions }} />
        </div>

        <div className="stats-card animate-section" style={{ animationDelay: '0.4s' }}>
          <MuscleWorkload />
        </div>

        <div className="stats-card animate-section" style={{ animationDelay: '0.5s' }}>
          <IntensityZones />
        </div>

        <div className="stats-card animate-section" style={{ animationDelay: '0.6s' }}>
          <MuscleHud />
        </div>
      </main>

      {/* FOOTER */}
      <footer className="footer-cockpit">
        <div className="text-[10px] text-gray-600 font-mono tracking-widest uppercase text-center">
          // END OF STREAM //
        </div>
      </footer>
    </div>
